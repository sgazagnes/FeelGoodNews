...
def main():
    """Main function to run the LLM-powered good news scraper"""
    print("=== Feel Good News with AI Images ===")
    ...
    print("\nAvailable countries:", list(scraper.news_sources.keys()))
    print("Available personalities: darth_vader, gary_lineker, drunk_philosopher, shakespeare, gordon_ramsay")
    print()
    
    # Get user input
    country = input("Enter country (or 'global'): ").strip().lower() or 'global'
    personality = input("Choose personality: ").strip().lower() or 'darth_vader'
    max_articles = int(input("Max articles to find (default 5): ").strip() or "5")
    generate_images = input("Generate AI images? (y/n, default y): ").strip().lower() != 'n'
    
    print(f"\n🚀 Starting LLM analysis for {country}...")
    articles = scraper.scrape_and_analyze_news(country, max_articles, generate_images=generate_images)
    
    if not articles:
        print("😞 No good news found. Try a different country or check your internet connection.")
        return
    
    print(f"\n🎉 Found {len(articles)} good news articles!")
    
    # Generate personality presentations
    presentations = scraper.present_news_with_personality(articles, personality)
    
    print("\n" + "="*80)
    print(f"📰 GOOD NEWS PRESENTED BY {personality.upper()}")
    print("="*80)
    
    for i, (article, presentation) in enumerate(zip(articles, presentations), 1):
        print(f"\n🌟 GOOD NEWS #{i}")
        print("="*50)
        print(presentation)
                                # # Generate image if requested
        if generate_images:
            ...
            print(f"\n🖼️  AI Generated Image: {article.image_url}")
            # print(f"🎨 Image Prompt: {article.image_prompt}")
        print(f"\n🔗 Read more: {article.url}")
        print(f"😊 AI Positivity Score: {article.sentiment_score:.2f}")
        print("-"*50)
    ...
    # Ensure scraped_news directory exists
    os.makedirs("scraped_news", exist_ok=True)
    filename = f"scraped_news/good_news_llm_{country}_{personality}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
